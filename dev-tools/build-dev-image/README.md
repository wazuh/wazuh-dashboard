...
| `--opensearch-version` | `-o` | OpenSearch Dashboard version | `3.1.0.0` |
| `--wazuh-branch` | `-w` | Wazuh Dashboard branch | `migrate-main-to-3.1.0` |
| `--security-branch` | `-s` | Wazuh Dashboard Security branch | `migrate-main-to-3.1.0.0` |
| `--reporting-branch` | `-r` | Wazuh Dashboard Reporting branch | `migrate-main-to-3.1.0.0` |
| `--plugins-branch` | `-p` | Wazuh Dashboard Plugins branch | `change/7560-compatibility-with-opensearch-310` |
| `--tag` | `-t` | Docker image tag | `3.1.0-5.0.0` |
...
docker build --platform linux/arm64 \
  ...
  --build-arg OPENSEARCH_DASHBOARD_VERSION=3.1.0.0 \
  --build-arg WAZUH_DASHBOARD_BRANCH=main \
  -t quay.io/wazuh/osd-dev:3.1.0-arm64 \
  -f wzd.dockerfile .
...
docker build --platform linux/amd64 \
  ...
  --build-arg OPENSEARCH_DASHBOARD_VERSION=3.1.0.0 \
  --build-arg WAZUH_DASHBOARD_BRANCH=main \
  -t quay.io/wazuh/osd-dev:3.1.0-amd64 \
  -f wzd.dockerfile .
```
