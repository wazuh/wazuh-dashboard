...
run_with_retry env GIT_REF="${WAZUH_DASHBOARD_SECURITY_ANALYTICS_BRANCH}" yarn install
echo "Building Wazuh Security Analytics plugin"
run_with_retry env GIT_REF="${WAZUH_DASHBOARD_SECURITY_ANALYTICS_BRANCH}" yarn build
