...
assert_file_contains "$WORK/package.json" '"version": "3.6.0"' \
  "package.json: top-level OpenSearch Dashboards version is NOT touched"
...
WF1="$WORK/.github/workflows/5_builderpackage_dashboard_core.yml"
WF2="$WORK/.github/workflows/5_builderpackage_dev_docker_image.yml"
assert_file_contains "$WF1" "default: '5.1.0'" "dashboard_core workflow: branch default bumped"
assert_file_contains "$WF2" "default: '5.1.0'" "dev_docker_image workflow: branch default bumped"

README="$WORK/dev-tools/build-packages/base-packages-to-base/README.md"
assert_file_contains "$README" "--app 5.1.0" "base-packages README: --app example bumped"
assert_file_contains "$README" "--base 5.1.0" "base-packages README: --base example bumped"
assert_file_contains "$README" "--security 5.1.0" "base-packages README: --security example bumped"
assert_file_contains "$README" "--securityAnalytics 5.1.0" "base-packages README: --securityAnalytics example bumped"
assert_file_contains "$README" "--reporting 5.1.0" "base-packages README: --reporting example bumped"
assert_file_contains "$README" "--alerting 5.1.0" "base-packages README: --alerting example bumped"
assert_file_contains "$README" "--notifications 5.1.0" "base-packages README: --notifications example bumped"
assert_file_contains "$README" "divided by repository of the 5.1.0 branch" \
  "base-packages README: prose example bumped"

SNAP="$WORK/src/core/server/rendering/__snapshots__/rendering_service.test.ts.snap"
assert_file_contains "$SNAP" '"wazuhVersion": "5.1.0"' "rendering snapshot: wazuhVersion bumped"

HEALTHCHECK="$WORK/src/core/server/healthcheck/healthcheck/config.ts"
assert_file_contains "$HEALTHCHECK" "documentation.wazuh.com/5.1/" \
  "healthcheck troubleshooting link: minor version bumped"

MULTIARCH="$WORK/dev-tools/build-dev-image/build-multiarch.sh"
assert_file_contains "$MULTIARCH" 'OPENSEARCH_DASHBOARD_VERSION="3.6.0.0"' \
  "build-multiarch.sh: OPENSEARCH_DASHBOARD_VERSION uses platform version"
assert_file_contains "$MULTIARCH" 'TAG="3.6.0-5.1.0"' "build-multiarch.sh: TAG combines platform + wazuh version"
assert_file_contains "$MULTIARCH" 'NODE_VERSION="22.15.0"' "build-multiarch.sh: NODE_VERSION from .nvmrc"

DEV_README="$WORK/dev-tools/build-dev-image/README.md"
assert_file_contains "$DEV_README" "--build-arg OPENSEARCH_DASHBOARD_VERSION=3.6.0.0" \
  "build-dev-image README: OPENSEARCH_DASHBOARD_VERSION build-arg bumped"
assert_file_contains "$DEV_README" "--build-arg NODE_VERSION=22.15.0" \
  "build-dev-image README: NODE_VERSION build-arg bumped"
assert_file_contains "$DEV_README" "quay.io/wazuh/osd-dev:3.6.0-a" "build-dev-image README: -a tag bumped"
assert_file_contains "$DEV_README" "quay.io/wazuh/osd-dev:3.6.0-5.1.0" "build-dev-image README: full tag bumped"
...
assert_file_contains "$WORK/src/core/server/healthcheck/healthcheck/config.ts" \
  "documentation.wazuh.com/5.0/" "healthcheck link untouched (minor version unchanged)"
...
assert_file_contains "$WORK/package.json" '"version": "5.0.0"' "package.json untouched"
...
WF1="$WORK/.github/workflows/5_builderpackage_dashboard_core.yml"
assert_file_contains "$WF1" "default: 'v5.0.0-rc1'" "workflow: tag ref with stage"
assert_file_contains "$WORK/dev-tools/build-packages/base-packages-to-base/README.md" \
  "--app v5.0.0-rc1" "base-packages README: tag ref with stage"
assert_file_contains "$WORK/CHANGELOG.md" "Some existing feature entry" \
...
WF1="$WORK/.github/workflows/5_builderpackage_dashboard_core.yml"
assert_file_contains "$WF1" "default: 'v5.0.0'" "workflow: stageless tag ref"
assert_file_contains "$WORK/dev-tools/build-packages/base-packages-to-base/README.md" \
  "--app v5.0.0" "base-packages README: stageless tag ref"
...
WF1="$WORK/.github/workflows/5_builderpackage_dashboard_core.yml"
WF2="$WORK/.github/workflows/5_builderpackage_dev_docker_image.yml"
assert_file_contains "$WF1" "default: 'main'" "dashboard_core workflow: default still main"
assert_file_contains "$WF2" "default: 'main'" "dev_docker_image workflow: default still main"
...
# update_readme_for_base_packages is not gated by --set-as-main: it always
# reflects the target version, since it documents build commands rather
# than branch checkout refs.
assert_file_contains "$WORK/dev-tools/build-packages/base-packages-to-base/README.md" \
  "--app 5.1.0" "base-packages README examples are bumped even under --set-as-main"
