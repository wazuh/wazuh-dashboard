...
CURRENT_VERSION=""
TAG=false
WAZUH_DASHBOARD_PLUGINS_WORKFLOW_FILE="${REPO_PATH}/.github/workflows/6_builderpackage_dashboard.yml"
...
README_FOR_DEV_IMAGE="${REPO_PATH}/dev-tools/build-dev-image/README.md"
DOCKERFILE_FOR_BUILD_DEV_IMAGE="${REPO_PATH}/dev-tools/build-dev-image/wzd.dockerfile"
...
update_readme_for_base_packages() {
  ...
  if [ -f "$README_FOR_BASE_PACKAGES" ]; then
    ...
    # Update all occurrences of --reporting x.y.z with --reporting $VERSION
    # Define the pattern regex for --reporting
    local reporting_pattern_regex="(--reporting )$VERSION_PATTERN"
    ...
    if grep -qE "$reporting_pattern_regex" "$README_FOR_BASE_PACKAGES"; then
      ...
      # If the pattern exists, perform the substitution and set modified to true
      sed -i -E "s/${reporting_pattern_regex}/\1${VERSION}/g" "$README_FOR_BASE_PACKAGES"
      modified=true
    else
...
update_readme_for_dev_image() {
  log "Updating $(basename $README_FOR_DEV_IMAGE)..."

  if [ -f "$README_FOR_DEV_IMAGE" ]; then
    local modified=false

    # Update all occurrences of x.y.z with $VERSION
    # Define the pattern regex
    local app_pattern_regex="$CURRENT_VERSION"
    # Check if the pattern exists in the file
    ...
    if [[ $modified == true ]]; then
      log "Successfully updated $(basename $README_FOR_DEV_IMAGE)"
    fi
  fi
}

update_dockerfile_for_build_dev_image() {
  log "Updating $(basename $DOCKERFILE_FOR_BUILD_DEV_IMAGE)..."

  if [ -f "$DOCKERFILE_FOR_BUILD_DEV_IMAGE" ]; then
    local modified=false
    ...
    if [[ $modified == true ]]; then
      log "Successfully updated $(basename $DOCKERFILE_FOR_BUILD_DEV_IMAGE)"
    fi
  fi
}
