...
- [ ] 3. Run check-standards (lint + format + typecheck + tests) and fix failures
...
Invoke the **check-standards** skill (prettier + eslint + typecheck on changed
files, tests for touched code). Do not proceed to "Ready for review" until it
passes.
...
- CHANGELOG: entry added (links to issue) / not needed (internal / no user impact)
