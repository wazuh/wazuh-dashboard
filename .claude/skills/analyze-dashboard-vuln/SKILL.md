...
> **repo-specific (local environment):** the Wazuh dashboard repos sit side by side under the workspace root (on this machine `/home/asus/wazuh/`). This repo, `wazuh-dashboard`, is the platform (upstream `OpenSearch-Dashboards`). Write scratch files and issue drafts to a git-ignored dir such as `tmp/vuln-issues/`.
