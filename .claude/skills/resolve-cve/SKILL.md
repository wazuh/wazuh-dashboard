...
> **`yarn osd bootstrap`**. Forced versions live in the **root `package.json` > `resolutions`** block.
> Tests generally run **on the host** here (the full OSD checkout ships `setup_node_env`).
